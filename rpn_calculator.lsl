...
{    
    ...
    state_entry()
    {
       ...
       listen_handle = llListen(0, NULL_STRING, llGetOwner(), "/calc2");
    } 
 ...
 state_entry()
 {
   ...
   while (count < lLength) 
   {
       ...
       else
       {
          ...
          if(llSubStringIndex(CALCULATIONS, temp) == MOD)
          {stack = ((integer)e2 % (integer)e1) + stack;
          debug((string)e2 + " mod " + (string)e1);
          }        
